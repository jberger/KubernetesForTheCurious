        ...
        return Promise.all([...document.querySelectorAll('code[data-url]')].map((el) => {
            ...
            return fetch(url)
                .then((r) => {
                    ...
                    const noSource = el.hasAttribute('data-no-source');
                    ...
                    reveal.getPlugin('highlight').highlightBlock(el);

                    // add source reference
                    if (! noSource) {
                      el.parentNode.insertAdjacentHTML('afterend', `<p class="code-source">${el.dataset.url}</p>`);
                    }
